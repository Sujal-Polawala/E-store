...
exports.downloadOrder = async (req, res) => {
  ...
  try {
    ...
    doc
      .fontSize(10)
      .text("Your Company Address", 110, 70)
      .text("City, State, ZIP Code", 110, 85)
      .text("Phone: (123) 456-7890", 110, 100)
      .text("Email: contact@yourcompany.com", 110, 115)
      .moveDown();
    ...
    doc
      .fontSize(12)
      .text(`Invoice Number: ${order._id}`, 50, 180)
      .text(`Date: ${new Date(order.createdAt).toLocaleDateString()}`, 50, 200)
      .text(`Total: ₹${order.totalPrice.toFixed(2)}`, 50, 220);

    doc
      .text(`Billed To: ${order.userId.firstname} ${order.userId.lastname}`, 300, 180)
      .text(`Address:`, 300, 200)
      .text(address, { width: 250, align: "left" })
      .moveDown();
