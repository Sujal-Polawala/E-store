...
const SuccessPage = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      if (!localStorage.getItem("popupShown")) {
        setPopup({
          message: `Payment successful! Order Status: ${response.data.status}, Estimated Delivery: ${new Date(response.data.deliveryDate).toDateString()}`,
          type: "success",
          show: true,
        });
  ...
  if (!orderDetails) {
    return <div className="h-screen flex justify-center items-center text-xl font-semibold">No order details found!</div>;
  }
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-gradient-to-br from-gray-100 to-gray-300 px-6 py-10 animate-fade-in">
      ...
      <div className="bg-white shadow-2xl rounded-2xl p-12 w-full max-w-4xl text-center transform transition duration-300 hover:scale-105 flex-grow">
        ...
        <h1 className="text-4xl font-extrabold text-gray-900">Payment Successful!</h1>
        <p className="text-gray-600 mt-3 text-lg">Your order has been placed successfully.</p>
        ...
        <div className="mt-8 p-6 bg-gray-50 rounded-lg text-left shadow-md">
          ...
          <ul className="space-y-4">
            {orderDetails.items.map((item) => (
              <li key={item._id} className="p-4 bg-white rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="flex justify-between text-lg">
                  <span className="font-semibold text-gray-700">Quantity x Price:</span>
        ...
        {/* Buttons */}
        <div className="mt-8 flex flex-col space-y-4 w-full pb-20"> {/* Added padding-bottom */}
          <button
